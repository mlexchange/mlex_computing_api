...
def post_job(job_uid, job_status, job_pid=None):
    url = 'http://job-dispatcher:8081/api/v0/jobs/' + job_uid + '/status' #?status='+ job_status
    if job_pid is None:
    ...
    **Example:**
    ...
    def _process_message(self, msg: Message) -> None:
        ...
        try:
            ...
            job_repr = get_call_repr(client.containers.run, *job.args) #, **job.kwargs)
        except Exception as err:
            ...
            try:
                ...
                self._logger.info(f"Job {job.id} returned: {res.id}")
                self._execute_callback("success", msg, job, res, None, None)
                post_job(job.id, "running", res.id)
            finally:
